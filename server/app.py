...
class Game:
    ...
    def drop_piece(self, player_color, pid, x,y):
        ...
        if not self.pos_empty(x,y): return False, "target occupied"
        # pawn drop restrictions (rule 4,15)
        p = self.pieces[pid]
        if p.type=='pawn':
            ...
            # set pawn stun by rank mapping (rule 15)
            if player_color=='w':
                p.stun = y if y<=6 else 1
            else:
                p.stun = (7-y) if y>=1 else 1
        else:
            # default stun 1 for non-pawn drop
            p.stun = max(1, p.stun)
        p.on_drop((x,y))
        self.board[y][x] = pid
        self.hands[player_color].remove(pid)
        self.history.append({"action":"drop","player":player_color,"piece":pid,"to":[x,y]})
    ...
    def move_piece(self, player_color, pid, frm, to):
        ...
        if self.board[y1][x1] != pid: return False,"source mismatch"
        piece = self.pieces[pid]
        if piece.color != player_color: return False,"not your piece"
        if piece.stun > 0:
            return False, f"piece stunned {piece.stun}"
        # movement rule check
        if not piece.can_move(frm,to,self._board_piece_objects()):
            return False, "illegal move for piece"
        # suicide check (simplified): don't allow move that captures your own king or leaves your king absent
        # capture?
        target_pid = self.board[y2][x2]
        if target_pid is not None:
            target = self.pieces[target_pid]
            if target.color == piece.color:
                return False, "cannot capture own piece"
            # capture: transfer stacks
            piece.on_capture(target)
            # remove target to player's hand
            target.pos = None; target.captured=True; target.stun=0; target.move_stack=0
            self.hands[player_color].append(target_pid)
            # clear board at target
    ...
    def _board_piece_objects(self):
        # helper to create 2d board of Piece instances or None for movement checks
    ...
    def is_safe_after_move(self, pid, frm, to):
        # simplified check: after applying this move on a copy, ensure your king still exists (no self-capture)
        gcopy = copy.deepcopy(self)
        # apply move in copy
        ok, msg = gcopy.move_piece(gcopy.pieces[pid].color, pid, frm, to)
        if not ok:
            return False
        # ensure own king exists on board
        own_color = self.pieces[pid].color
        king_exists = False
...
def on_connect():
    ...
    # send initial state
    emit('game_state', game.serialize())
...
def on_move_request(data):
    ...
    # legality
    if not piece.can_move(frm,to,game._board_piece_objects()):
        emit('move_rejected', {'reason':'illegal_move'}, to=sid); return
    # suicide check
    if not game.is_safe_after_move(pid, frm, to):
    ...
    socketio.emit('game_state', game.serialize())
